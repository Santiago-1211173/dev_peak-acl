...
def test_message_template_match_is_case_insensitive():
    template = MessageTemplate(performative="inform", protocol="FIPA-REQUEST", ontology="HeAlTh")
    acl = AclMessage(
        performative="INFORM",
...
async def test_conversation_manager_request_agree_inform_flow():
    ...
    inform = AclMessage("inform", conversation_id=sent_msg.conversation_id, content="done")
...
async def test_conversation_manager_request_refuse_finishes_immediately():
    ...
    fut = await manager.send_request(sender=sender, receiver=receiver, content="x", url="http://receiver/acc")
